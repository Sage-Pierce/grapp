...
public final class GrappStoreLayoutDTOOFactory {
   ...
   public static GrappStoreLayoutDTOO createDTO(GrappStoreLayout grappStoreLayout) {
      ...
      grappStoreLayoutDTO.setFeatures(grappStoreLayout.getFeatures().stream().map(GrappStoreFeatureDTOOFactory::createDTO).collect(Collectors.toList()));
      grappStoreLayoutDTO.setNodes(grappStoreLayout.getNodes().stream().map(GrappStoreNodeDTOOFactory::createDTO).collect(Collectors.toList()));
      return grappStoreLayoutDTO;
   }

   private GrappStoreLayoutDTOOFactory() {
